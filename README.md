...
Gave claude the the different tools and their descriptions from above and aks it to implement the function. I verified it filtered by all three
parameters and returned `[]` on no match before running it. I then tested it with three queries
(results case, impossible constraints case, price filter case) and all passed.
**Milestone 4 — Planning loop and state management:**
Give claude the architecture and state management and ask it to implement run_agent. Check differnet types of queries to make sure it returns errors and responses both.Claude produced the planning loop with the correct session dict structure and early-return branch on empty search results. I checked that it branched on the search result before running it, and confirmed the no-results path set `session["error"]` and left
...
Spec Reflection 
The LLM prompt for create_fit_card went through several iterations to control output length. The initial AI prompting was leading to captions that were too long, however, when i cut the number of tokens, it would cut off sentences in the middle so i had to specify not to do that. 